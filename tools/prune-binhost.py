...
superseded tbz2s) and writes a sorted `.manifest` of what the overlay currently
...
Usage: prune-binhost.py [--binhost DIR] [--manifest FILE] [--dry-run]
"""
...
def main() -> None:
    ...
    if removed:
        print(f"pruned {len(removed)} superseded binpkg(s)")
        print("overlay is NOT a no-op: content changed")
    else:
        print("no superseded binpkg(s); manifest unchanged if versions did not move")
